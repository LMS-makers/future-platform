...
export default function RightPanel({ schedule, tasks, gpa, cgpa }: RightPanelProps) {
  ...
  return (
    <div className="w-full lg:w-80 space-y-6 lg:space-y-8">
      <div className="bg-white dark:bg-slate-800 p-5 lg:p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
        ...
        <div className="flex items-center justify-between mt-5 lg:mt-6 pt-4 border-t border-slate-50 dark:border-slate-700">
          ...
          {schedule.length > 0 ? (
            schedule.map((item) => (
              <div key={item.day} className="flex items-center gap-3 lg:gap-4">
                <div className="flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-700 rounded-xl w-10 lg:w-12 shrink-0">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">{item.day}</span>
                  <span className="text-xs lg:text-sm font-bold text-slate-900 dark:text-white">{item.date}</span>
                </div>
                <div className="min-w-0 flex-1">
                  <h4 className="text-xs font-bold text-slate-900 dark:text-white truncate">{item.title}</h4>
                  <p className="text-[10px] text-slate-400 truncate">{item.time} &bull; {item.location}</p>
                </div>
              </div>
            ))
          ) : (
            <p className="text-[10px] text-slate-400 text-center py-4">No schedule available</p>
          )}
        </div>
        ...
        <div className="space-y-2">
          {tasks.length > 0 ? (
            tasks.map((task) => (
              <div
                key={task.id}
                className="bg-white dark:bg-slate-800 p-3 lg:p-4 rounded-xl border border-slate-100 dark:border-slate-700 flex items-center justify-between group cursor-pointer"
              >
                ...
                <span className="material-symbols-outlined text-slate-300 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors text-lg shrink-0">
                  chevron_right
                </span>
              </div>
            ))
          ) : (
            <p className="text-[10px] text-slate-400 text-center py-4">No tasks due</p>
          )}
        </div>
