...
function SkeletonCard() {
  return (
    <div className="bg-surface-card rounded-2xl p-6 shadow-md animate-pulse">
    ...
    <section className="py-20 bg-surface">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex justify-center mb-12">
          <div className="inline-flex bg-surface-alt rounded-xl p-1.5">
            {tabs.map(tab => (
              <button
                key={tab.key}
        ...
        {error && (
          <div className="text-center py-12">
            ...
            <p className="text-text-tertiary mt-2">{error}</p>
          </div>
        )}
            ...
            {loading
              ...
              : (data as any[])?.map((person: any) => (
                  <motion.div
                    ...
                    transition={{ duration: 0.3 }}
                    className="group bg-surface-card rounded-xl p-6 shadow-md hover:shadow-lg transition-all duration-300 border border-border-muted hover:border-primary-200"
                  >
                    <div className="flex items-center gap-4 mb-4">
					  <img
							src={person.avatar}
                    ...
                    <p className="text-text-tertiary text-sm leading-relaxed mb-4">{person.bio}</p>
                    {person.stats && (
                      <div className="flex gap-3 pt-4 border-t border-border-muted">
                        {person.stats.map((stat: any) => (
                          <div key={stat.label} className="flex-1 text-center">
                            <div className="text-lg font-bold text-text-primary">{stat.value}</div>
                            <div className="text-text-tertiary text-xs">{stat.label}</div>
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    {person.publications !== undefined && (
                      <div className="flex items-center gap-1.5 text-text-tertiary text-xs">
                        <span className="font-semibold text-text-secondary">{person.publications}</span> publications
                      </div>
                    )}
                  </motion.div>
                ))}
